...
// func ReplaceTransactionWithHigherFee(w *wallet.Wallet, service *neutrino.ChainService, originalTxID string, newFeeRate int64, electrumClient *electrum.Client) (chainhash.Hash, error) {
// 	log.Printf("Starting RBF process for transaction %s with new fee rate %d sat/vB", originalTxID, newFeeRate)

// 	// Unlock wallet
// 	walletPassphrase := os.Getenv("PRIVATE_PASSPHRASE")
// 	log.Printf("Unlocking wallet.")
// 	err := w.Unlock([]byte(walletPassphrase), nil)
// 	if err != nil {
// 		log.Printf("Failed to unlock wallet: %v", err)
// 		return chainhash.Hash{}, fmt.Errorf("failed to unlock wallet: %v", err)
// 	}
// 	defer w.Lock()
...
// 		return chainhash.Hash{}, fmt.Errorf("error fetching transaction: %v", err)
// 	}

// 	// Decode the original transaction
// 	txBytes, err := hex.DecodeString(txDetails)
// 	if err != nil {
// 		return chainhash.Hash{}, fmt.Errorf("failed to decode transaction hex: %v", err)
// 	}
// 	originalTx := wire.NewMsgTx(wire.TxVersion)
// 	if err := originalTx.Deserialize(bytes.NewReader(txBytes)); err != nil {
// 		return chainhash.Hash{}, fmt.Errorf("failed to deserialize original transaction: %v", err)
// 	}
...
// 		utxo, err := fetchUTXO(w, &txIn.PreviousOutPoint)
// 		if err != nil {
// 			return chainhash.Hash{}, fmt.Errorf("failed to get UTXO for input: %v", err)
// 		}
// 		totalIn += int64(utxo.Amount * btcutil.SatoshiPerBitcoin)
// 	}
...
// 	if extraFee <= 0 {
// 		return chainhash.Hash{}, fmt.Errorf("new fee is not higher than the original fee")
// 	}

// 	// Adjust the last output (change) to accommodate the new fee
// 	lastOutputIndex := len(newTx.TxOut) - 1
// 	if btcutil.Amount(newTx.TxOut[lastOutputIndex].Value) < extraFee {
// 		return chainhash.Hash{}, fmt.Errorf("not enough funds in the change output to cover the new fee")
// 	}
// 	newTx.TxOut[lastOutputIndex].Value -= int64(extraFee)
...
// 	txWeight := newTx.SerializeSizeStripped()*3 + newTx.SerializeSize()
// 	if txWeight > MaxStandardTxWeight {
// 		return chainhash.Hash{}, fmt.Errorf("transaction weight (%d) exceeds maximum allowed (%d)", txWeight, MaxStandardTxWeight)
// 	}
...
// 		utxo, err := fetchUTXO(w, &txIn.PreviousOutPoint)
// 		if err != nil {
// 			return chainhash.Hash{}, fmt.Errorf("failed to get UTXO for input %d: %v", i, err)
// 		}

// 		scriptPubKey, err := hex.DecodeString(utxo.ScriptPubKey)
// 		if err != nil {
// 			return chainhash.Hash{}, fmt.Errorf("failed to decode scriptPubKey for input %d: %v", i, err)
// 		}
...
// 		_, addrs, _, err := txscript.ExtractPkScriptAddrs(scriptPubKey, w.ChainParams())
// 		if err != nil {
// 			return chainhash.Hash{}, fmt.Errorf("failed to extract address for input %d: %v", i, err)
// 		}

// 		if len(addrs) == 0 {
// 			return chainhash.Hash{}, fmt.Errorf("no addresses found for input %d", i)
// 		}
...
// 		privKey, err := w.PrivKeyForAddress(addrs[0])
// 		if err != nil {
// 			return chainhash.Hash{}, fmt.Errorf("failed to get private key for input %d: %v", i, err)
// 		}
...
// 			witnessScript, err := txscript.WitnessSignature(newTx, txscript.NewTxSigHashes(newTx, prevOutputFetcher), i, inputAmount, scriptPubKey, txscript.SigHashAll, privKey, true)
// 			if err != nil {
// 				return chainhash.Hash{}, fmt.Errorf("failed to create witness script for input %d: %v", i, err)
// 			}
// 			newTx.TxIn[i].Witness = witnessScript
// 			log.Printf("Witness script created for input %d", i)
// 		} else {
...
// 			sigScript, err := txscript.SignatureScript(newTx, i, scriptPubKey, txscript.SigHashAll, privKey, true)
// 			if err != nil {
// 				return chainhash.Hash{}, fmt.Errorf("failed to create signature script for input %d: %v", i, err)
// 			}
// 			newTx.TxIn[i].SignatureScript = sigScript
// 			log.Printf("Signature script created for input %d", i)
// 		}
...
// 		if err != nil {
// 			log.Printf("Signature verification failed for input %d: %v", i, err)
// 			return chainhash.Hash{}, fmt.Errorf("failed to verify signature for input %d: %v", i, err)
// 		}
// 		if !valid {
// 			log.Printf("Invalid signature for input %d", i)
// 			return chainhash.Hash{}, fmt.Errorf("signature verification failed for input %d", i)
// 		}
// 		log.Printf("Signature verification succeeded for input %d", i)
// 	}
...
// 	// Broadcast and verify the transaction
// 	txHash, err := broadcastAndVerifyTransaction(newTx, service)
// 	if err != nil {
// 		return chainhash.Hash{}, fmt.Errorf("failed to broadcast and verify RBF transaction: %v", err)
// 	}
